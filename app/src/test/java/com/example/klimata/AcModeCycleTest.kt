...
class ACModeCycleTest {
    ...
    @Test
    fun provisionedRooms_containValidNativeAcModes() {
        val validModes = setOf("Cool", "Dry", "Fan", "Auto")
        val rooms = listOf(
            com.example.klimata.data.RoomFactory.createRoom("Master Bed", 20, 2.8f),
            com.example.klimata.data.RoomFactory.createRoom("Living Room", 35, 3.0f),
            com.example.klimata.data.RoomFactory.createRoom("Study", 14, 2.7f)
        )
        rooms.forEach { room ->
            assertTrue(
